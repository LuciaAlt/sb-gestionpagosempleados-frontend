...
import logo from '../../assets/logo-sliderv1.png';

export default function AppLayout() {
  const { user, logout, can } = useAuth();
  const [profile, setProfile] = useState(false);
  const name =
    user?.nombreCompleto || user?.nombreUsuario || user?.usuario || "Usuario";
  return (
    <div className="app-layout">
      <aside className="sidebar">
        <img src={logo} alt="SB" className="sidebar-logo" />
        <nav>
      ...
      <main className="main">
        <header className="topbar">
          <h3>Sistema de Gestión de Pagos de Empleados</h3>
          <div className="user-menu">
            <button className="user-name" onClick={() => setProfile(true)}>
              {name}
            </button>
      ...
      <UserProfileModal show={profile} onHide={() => setProfile(false)} />
    </div>
  );
}
function NavItem({
