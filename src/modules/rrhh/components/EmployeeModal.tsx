...
type Props = {
  show: boolean;
  onHide: () => void;
  onSave: (payload: EmployeeRequest) => Promise<void>;
...
export default function EmployeeModal({
      ...
      setForm(
        employee
          ? {
              ...
              ...employee,
              departamentoId:
                employee.departamentoId || employee.departamentoId,
            }
          : empty
      );
    setValidated(false);
  ...
  const selectedType = useMemo(
    () => types.find((t) => t.id === Number(form.tipoEmpleadoId)),
    [types, form.tipoEmpleadoId]
  );
  const typeName = (selectedType?.nombre || "").toLowerCase();
  const showWeekly =
    typeName.includes("asalariado") &&
    !typeName.includes("comisión") &&
    !typeName.includes("comision");
  const showHourly = typeName.includes("hora");
  const showCommission =
    typeName.includes("comisión") || typeName.includes("comision");
  const showBase = showCommission && typeName.includes("asalariado");
  const cleanAmounts = (payload: EmployeeRequest): EmployeeRequest => ({
    ...payload,
  ...
  const submit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      await onSave(cleanAmounts(form));
      onHide();
    } finally {
      setSaving(false);
    }
  };
  const set = (k: keyof EmployeeRequest, v: any) =>
          ...
          <Row className="g-3">
            {employee && (
              <Col md={3}>
                ...
                <Form.Control value={employee.id} disabled />
              </Col>
            )}
            <Col md={employee ? 5 : 6}>
              <Form.Label>
            ...
            <Col md={6}>
              <Form.Label>
                <RequiredLabel>Número Seguro Social</RequiredLabel>
              </Form.Label>
              <Form.Control
                required
                disabled={readonly}
            ...
            <Col md={6}>
              <Form.Label>
                <RequiredLabel>Tipo Empleado</RequiredLabel>
              </Form.Label>
              <Form.Select
                required
                disabled={readonly}
                value={form.tipoEmpleadoId || ""}
                onChange={(e) => set("tipoEmpleadoId", Number(e.target.value))}
              >
                <option value="">Seleccione</option>
                {types.map((x) => (
                  <option key={x.id} value={x.id}>
            ...
            {showWeekly && (
              <MoneyField
                ...
                onChange={(v) => set("salarioSemanal", v)}
                readonly={readonly}
              />
            )}
            {showHourly && (
              <>
                <MoneyField
                  ...
                  onChange={(v) => set("sueldoPorHora", v)}
                  readonly={readonly}
                />
                <MoneyField
                  label="Horas Trabajadas"
                  value={form.horasTrabajadas}
                  onChange={(v) => set("horasTrabajadas", v)}
                  readonly={readonly}
                />
              </>
            )}
            {showCommission && (
              <>
                <MoneyField
                  ...
                  onChange={(v) => set("ventasBrutas", v)}
                  readonly={readonly}
                />
                <MoneyField
                  label="Tarifa Comisión"
                  value={form.tarifaComision}
                  onChange={(v) => set("tarifaComision", v)}
                  readonly={readonly}
                />
              </>
            )}
